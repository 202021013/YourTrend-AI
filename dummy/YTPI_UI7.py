...
def generate_idea(transcripts: List[str], video_urls: List[str], user_prompt: str):
    try:
...
def search_videos(keyword: str, duration_category: str, sort_by='relevance') -> pd.DataFrame:
    videos_search = VideosSearch(keyword, limit=10)
    results = videos_search.result()
    videos = []
    
    for video in results['result']:
        video_duration_sec = parse_duration(video.get('duration', ''))
        ...
        if duration_included:
            view_count = safe_int_conversion(video.get('viewCount', {}).get('text', ''))
            videos.append({
                'video_id': video['id'],
                'title': video['title'],
                ...
                'view_count': video.get('viewCount', {}).get('text', 'Unknown'),
...
def save_idea(video_urls: List[str], transcript: str, idea: str):
    try:
        conn = sqlite3.connect('project_ideas.db')
        c = conn.cursor()
...
def format_views(view_count: str) -> str:
    ...
    try:
        ...
        if 'k' in cleaned_count:
            num = float(cleaned_count.replace('k', ''))
            return f"{num}K"
        elif 'm' in cleaned_count:
            num = float(cleaned_count.replace('m', ''))
            return f"{num}M"
        else:
                ...
                if st.button('선택한 영상으로 아이디어 생성하기'):
                    if not user_prompt.strip():
                        ...
                        with st.spinner('아이디어를 생성하고 있습니다...'):
                            combined_transcript, generated_idea = generate_idea_from_videos(
                                st.session_state.selected_videos, 
                                user_prompt
                            )
                            
                            if generated_idea:
                                st.session_state.generated_idea = generated_idea
