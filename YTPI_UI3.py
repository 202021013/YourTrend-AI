...
def generate_idea(transcripts, video_urls):
    try:
        ...
        analyst = AIAgent(
            ...
            주어진 영상 스크립트를 분석하여 트렌드, 패턴, 시장 기회를 파악합니다.
            시청자 반응, 콘텐츠 특성, 차별화 요소 등을 종합적으로 분석하여 인사이트를 도출합니다.""",
            temperature=0.3
        )
        ...
        practitioner = AIAgent(
            "실무자", 
            """당신은 프로젝트 실무 전문가입니다.
            분석된 내용을 바탕으로 실제 구현 가능한 프로젝트 아이디어를 제시합니다.
            기술적 실현 가능성, 리소스 효율성, 시장성을 고려하여 구체적인 실행 방안을 제시합니다.""",
            temperature=0.7
        )
        ...
        # 최종 아이디어 도출
        custom_prompt = """당신은 YouTube 콘텐츠 기반 프로젝트 아이디어 전문가입니다.
분석가와 실무자의 대화를 종합하여 다음 형식으로 응답해주세요:
...
def generate_idea_from_videos(selected_videos):
    ...
    try:
        ...
        idea, combined_transcript = generate_idea(transcripts, selected_videos)
        ...
        if st.session_state.selected_videos:
            ...
            if len(st.session_state.selected_videos) > 0:
                ...
                if st.button('선택한 영상으로 아이디어 생성하기'):
                    selected_videos = st.session_state.selected_videos  # 선택된 영상 URL 리스트를 가져오는 함수
                    combined_transcript, generated_idea = generate_idea_from_videos(selected_videos)
